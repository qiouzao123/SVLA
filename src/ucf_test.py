...
def test(model, testdataloader, maxlen, prompt_text, gt, gtlabels, device, label_map=None, args=None):
    args = args or ucf_option.parser.parse_args()
    repeat_factor = 16 
    ...
    with torch.no_grad():
        for i, item in enumerate(testdataloader):
            ...
            _, logits1, logits2, shot_slices, _ = model(
            ...
            prob2 = (1.0 - F.softmax(
                logits2.reshape(-1, logits2.shape[-1])[:len_cur], dim=-1
            )[:, 0])
...
if __name__ == '__main__':
    ...
    model = SVLA(
        ...
        shot_sim_thresh=args.shot_sim_thresh, shot_min_len=args.shot_min_len, shot_layers=args.shot_layers,
        shot_gamma=args.shot_gamma, pi_floor=args.pi_floor,
        cfa_tau=args.cfa_tau, cfa_beta=args.cfa_beta,
        cfa_prefix_len=args.cfa_prefix_len, cfa_bottleneck=args.cfa_bottleneck,
        cfa_prefix_rank=args.cfa_prefix_rank, cfa_dropout=args.cfa_dropout
    )
    model.load_state_dict(torch.load(args.model_path, map_location=device))

    auc1, ap1, auc2, ap2 = test(
        model, test_loader, args.visual_length, prompt_text, gt, gtlabels, device, label_map, args
    )
