...
from Sva_test import test as test_fn
import Sva_option
...
def train(model, normal_loader, anomaly_loader, testloader, args, label_map, device):
    ...
    max_epoch = int(getattr(args, "max_epoch", 50))
    ...
    for epoch_idx in range(start_epoch, max_epoch + 1):
        ...
        for i in range(num_iterations):
            ...
            if (step_print % print_every == 0) or (step_print == num_iterations):
                ...
                print(
                    f"epoch: {epoch_idx:3d} | step: {step_print:3d} | "
                ...
                print(
                    f"Validation -> Current AUC1: {auc1:.4f} (Best: {best_auc1:.4f}) | "
                    f"Current AP1: {ap1:.4f} (Best: {best_ap1:.4f})"
                )
                print(
                    f"              Current AUC2: {auc2:.4f} (Best: {best_auc2:.4f}) | "
...
if __name__ == "__main__":
    ...
    normal_dataset = SvaDataset(args.visual_length, args.train_list, False, label_map, normal=True)
    normal_loader = DataLoader(normal_dataset, batch_size=args.batch_size, shuffle=True, drop_last=True)

    anomaly_dataset = SvaDataset(args.visual_length, args.train_list, False, label_map, normal=False)
