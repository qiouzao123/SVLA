...
def train(model, train_loader, test_loader, args, label_map, device):
    ...
    print_every = int(getattr(args, "print_every", 10))
    txtreg_weight = float(getattr(args, "txtreg_weight", 1e-1))

    pi_topk_base_div = int(getattr(args, "pi_topk_base_div", 32)) 
    pi_topk_k_min = int(getattr(args, "pi_topk_k_min", 1))

    base_lr = args.lr
    pi_lr_mult = float(getattr(args, "pi_lr_mult", 5.0))
    pi_wd = 0.0
    base_wd = float(getattr(args, "weight_decay", 0.01))

    pi_params = list(model.shot_density_head.parameters())
    other_params = [p for n, p in model.named_parameters() if not n.startswith("shot_density_head")]
    ...
    if getattr(args, "use_checkpoint", False) and os.path.exists(args.checkpoint_path):
        ...
        ap1_best = checkpoint.get("ap1", 0.0)
        ap2_best = checkpoint.get("ap2", checkpoint.get("ap", 0.0))

        print(
            f"[resume] start_epoch={start_epoch} (last_epoch={last_epoch}) | "
            f"best AP1={ap1_best:.4f}, AP2={ap2_best:.4f}"
        )
    ...
    for epoch_idx in range(start_epoch, max_epoch):
        ...
        for i, item in enumerate(train_loader):
            ...
            text_features_ori, logits1_full, logits2, shot_slices, _ = model(
                visual_feat, None, text_list, feat_lengths
            )
            ...
            step_print = i + 1
            if (step_print % print_every == 0) or (step_print == len(train_loader)):
                ...
                print(
                    f"epoch: {epoch_idx+1:3d} | step: {step_print:3d} | "
                ...
                print(f"[val] Current AP1: {AP1:.4f} (Best: {ap1_best:.4f})")
                print(f"[val] Current AP2: {AP2:.4f} (Best: {ap2_best:.4f})")
